...
namespace Veldrid.D3D12
{
    internal class D3D12CommandList : CommandList
    {
        ...
        private ID3D12BlendState _blendState;
        private Color4 _blendFactor;
        private ID3D12DepthStencilState _depthStencilState;
        private uint _stencilReference;
        private ID3D12RasterizerState _rasterizerState;
        private Vortice.Direct3D.PrimitiveTopology _primitiveTopology;
        private ID3D12InputLayout _inputLayout;
        private ID3D12VertexShader _vertexShader;
        private ID3D12GeometryShader _geometryShader;
        private ID3D12HullShader _hullShader;
        private ID3D12DomainShader _domainShader;
        private ID3D12PixelShader _pixelShader;
        ...
        private ID3D12Buffer[] _cbOut = new ID3D12Buffer[1];
        ...
        private readonly D3D12Sampler[] _vertexBoundSamplers = new D3D12Sampler[MaxCachedSamplers];
        private readonly D3D12Sampler[] _fragmentBoundSamplers = new D3D12Sampler[MaxCachedSamplers];
        ...
        private readonly List<D3D12Buffer> _submittedStagingBuffers = new List<D3D12Buffer>();
        ...
        private protected override void SetIndexBufferCore(DeviceBuffer buffer, IndexFormat format, uint offset)
        {
            if (_ib != buffer || _ibOffset != offset)
            {
                ...
                _context.IASetIndexBuffer(D3D12Buffer.Buffer, D3D12Formats.ToDxgiFormat(format), (int)offset);
            }
        }
            ...
            for (int i = 0; i < resources.Length; i++)
            {
                ...
                switch (rbi.Kind)
                {
                    ...
                    case ResourceKind.StructuredBufferReadWrite:
                        {
                            D3D12BufferRange range = GetBufferRange(resource, bufferOffset);
                            ID3D12UnorderedAccessView uav = range.Buffer.GetUnorderedAccessView(range.Offset, range.Size);
                    ...
                    case ResourceKind.TextureReadWrite:
                        ...
                        BindUnorderedAccessView(d3d11RWTexView.Target, null, D3D12RWTexView.UnorderedAccessView, uaBase + rbi.Slot, rbi.Stages, slot);
                        break;
        ...
        private D3D12BufferRange GetBufferRange(BindableResource resource, uint additionalOffset)
        {
            ...
            {
                return new D3D12BufferRange(d3d11Buff, additionalOffset, D3D12Buff.SizeInBytes);
            }
        ...
        private protected unsafe override void UpdateBufferCore(DeviceBuffer buffer, uint bufferOffsetInBytes, IntPtr source, uint sizeInBytes)
        {
            ...
            else if (useMap && updateFullBuffer) // Can only update full buffer with WriteDiscard.
            {
                MappedSubresource msb = _context.Map(
                     d3dBuffer.Buffer,
                     0,
                     D3D12Formats.VdToD3D11MapMode(isDynamic, MapMode.Write),
                     MapFlags.None);
                if (sizeInBytes < 1024)
                {
