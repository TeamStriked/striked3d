...
namespace Veldrid.D3D12
{
    internal class D3D12TextureView : TextureView
    {
        ...
        public ID3D12ShaderResourceView ShaderResourceView { get; }
        public ID3D12UnorderedAccessView UnorderedAccessView { get; }

        public D3D12TextureView(D3D12GraphicsDevice gd, ref TextureViewDescription description)
            : base(ref description)
        {
            ID3D12Device device = gd.Device;
            D3D12Texture d3dTex = Util.AssertSubtype<Texture, D3D12Texture>(description.Target);
            ShaderResourceViewDescription srvDesc = D3D12Util.GetSrvDesc(
                d3dTex,
            ...
            if ((d3dTex.Usage & TextureUsage.Storage) == TextureUsage.Storage)
            {
                ...
                uavDesc.Format = D3D12Formats.GetViewFormat(d3dTex.DxgiFormat);
