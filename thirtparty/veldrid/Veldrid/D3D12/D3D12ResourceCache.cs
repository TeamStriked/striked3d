...
{
    internal class D3D12ResourceCache : IDisposable
    {
        private readonly ID3D12Device _device;
        private readonly object _lock = new object();

        private readonly Dictionary<BlendStateDescription, ID3D12BlendState> _blendStates
            = new Dictionary<BlendStateDescription, ID3D12BlendState>();

        private readonly Dictionary<DepthStencilStateDescription, ID3D12DepthStencilState> _depthStencilStates
            = new Dictionary<DepthStencilStateDescription, ID3D12DepthStencilState>();

        private readonly Dictionary<D3D12RasterizerStateCacheKey, ID3D12RasterizerState> _rasterizerStates
            = new Dictionary<D3D12RasterizerStateCacheKey, ID3D12RasterizerState>();

        private readonly Dictionary<InputLayoutCacheKey, ID3D12InputLayout> _inputLayouts
            = new Dictionary<InputLayoutCacheKey, ID3D12InputLayout>();
        ...
        public void GetPipelineResources(
            ...
            out ID3D12DepthStencilState depthState,
            out ID3D12RasterizerState rasterState,
        ...
        private ID3D12BlendState GetBlendState(ref BlendStateDescription description)
        {
            Debug.Assert(Monitor.IsEntered(_lock));
            if (!_blendStates.TryGetValue(description, out ID3D12BlendState blendState))
            {
        ...
        private ID3D12BlendState CreateNewBlendState(ref BlendStateDescription description)
        {
            BlendAttachmentDescription[] attachmentStates = description.AttachmentStates;
            Vortice.Direct3D12.BlendDescription d3dBlendStateDesc = new Vortice.Direct3D12.BlendDescription();
            ...
            for (int i = 0; i < attachmentStates.Length; i++)
            {
                ...
                d3dBlendStateDesc.RenderTarget[i].SourceBlend = D3D12Formats.VdToD3D11Blend(state.SourceColorFactor);
                d3dBlendStateDesc.RenderTarget[i].DestinationBlend = D3D12Formats.VdToD3D11Blend(state.DestinationColorFactor);
                d3dBlendStateDesc.RenderTarget[i].BlendOperation = D3D12Formats.VdToD3D11BlendOperation(state.ColorFunction);
                d3dBlendStateDesc.RenderTarget[i].SourceBlendAlpha = D3D12Formats.VdToD3D11Blend(state.SourceAlphaFactor);
                d3dBlendStateDesc.RenderTarget[i].DestinationBlendAlpha = D3D12Formats.VdToD3D11Blend(state.DestinationAlphaFactor);
                d3dBlendStateDesc.RenderTarget[i].BlendOperationAlpha = D3D12Formats.VdToD3D11BlendOperation(state.AlphaFunction);
            }
        ...
        private ID3D12DepthStencilState GetDepthStencilState(ref DepthStencilStateDescription description)
        {
            Debug.Assert(Monitor.IsEntered(_lock));
            if (!_depthStencilStates.TryGetValue(description, out ID3D12DepthStencilState dss))
            {
        ...
        private ID3D12DepthStencilState CreateNewDepthStencilState(ref DepthStencilStateDescription description)
        {
            DepthStencilDescription dssDesc = new DepthStencilDescription
            {
                DepthFunc = D3D12Formats.VdToD3D11ComparisonFunc(description.DepthComparison),
        ...
        private DepthStencilOperationDescription ToD3D11StencilOpDesc(StencilBehaviorDescription sbd)
        {
            ...
            {
                StencilFunc = D3D12Formats.VdToD3D11ComparisonFunc(sbd.Comparison),
                StencilPassOp = D3D12Formats.VdToD3D11StencilOperation(sbd.Pass),
                StencilFailOp = D3D12Formats.VdToD3D11StencilOperation(sbd.Fail),
                StencilDepthFailOp = D3D12Formats.VdToD3D11StencilOperation(sbd.DepthFail)
            };
        }

        private ID3D12RasterizerState GetRasterizerState(ref RasterizerStateDescription description, bool multisample)
        {
            Debug.Assert(Monitor.IsEntered(_lock));
            D3D12RasterizerStateCacheKey key = new D3D12RasterizerStateCacheKey(description, multisample);
            if (!_rasterizerStates.TryGetValue(key, out ID3D12RasterizerState rasterizerState))
            {
        ...
        private ID3D12RasterizerState CreateNewRasterizerState(ref D3D12RasterizerStateCacheKey key)
        {
            RasterizerDescription rssDesc = new RasterizerDescription
            {
                CullMode = D3D12Formats.VdToD3D11CullMode(key.VeldridDescription.CullMode),
                FillMode = D3D12Formats.VdToD3D11FillMode(key.VeldridDescription.FillMode),
        ...
        private ID3D12InputLayout GetInputLayout(VertexLayoutDescription[] vertexLayouts, byte[] vsBytecode)
        {
            Debug.Assert(Monitor.IsEntered(_lock));
            ...
            if (!_inputLayouts.TryGetValue(tempKey, out ID3D12InputLayout inputLayout))
            {
        ...
        private ID3D12InputLayout CreateNewInputLayout(VertexLayoutDescription[] vertexLayouts, byte[] vsBytecode)
        {
            int totalCount = 0;
        ...
        public void Dispose()
        {
            foreach (KeyValuePair<BlendStateDescription, ID3D12BlendState> kvp in _blendStates)
            {
                kvp.Value.Dispose();
            }
            foreach (KeyValuePair<DepthStencilStateDescription, ID3D12DepthStencilState> kvp in _depthStencilStates)
            {
                kvp.Value.Dispose();
            }
            foreach (KeyValuePair<D3D12RasterizerStateCacheKey, ID3D12RasterizerState> kvp in _rasterizerStates)
            {
                kvp.Value.Dispose();
            }
            foreach (KeyValuePair<InputLayoutCacheKey, ID3D12InputLayout> kvp in _inputLayouts)
            {
                kvp.Value.Dispose();
            }
        }
        ...
        private struct D3D12RasterizerStateCacheKey : IEquatable<D3D12RasterizerStateCacheKey>
        {
            public RasterizerStateDescription VeldridDescription;
            public bool Multisampled;

            public D3D12RasterizerStateCacheKey(RasterizerStateDescription veldridDescription, bool multisampled)
            {
                VeldridDescription = veldridDescription;
                Multisampled = multisampled;
            }

            public bool Equals(D3D12RasterizerStateCacheKey other)
            {
