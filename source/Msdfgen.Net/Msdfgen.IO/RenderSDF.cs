...
namespace Msdfgen
{
    public static class Render
    {
        ...
        public static void Simulate8Bit(Bitmap<float> bitmap)
        {
            ...
            for (var x = 0; x < w; ++x)
            {
                var v = (byte) MathExtension.Clamp(bitmap[x, y] * 0x100, 0, 0xff);
                bitmap[x, y] = v / 255.0f;
            }
        }
        ...
        public static void Simulate8Bit(Bitmap<FloatRgb> bitmap)
        {
            ...
            for (var x = 0; x < w; ++x)
            {
                var r = (byte) MathExtension.Clamp(bitmap[x, y].R * 0x100, 0, 0xff);
                var g = (byte) MathExtension.Clamp(bitmap[x, y].G * 0x100, 0, 0xff);
                var b = (byte) MathExtension.Clamp(bitmap[x, y].B * 0x100, 0, 0xff);
