...
namespace Striked3D.Types
{
        ...
        where T : unmanaged, IFormattable, IEquatable<T>, IComparable<T>
    {
        ...
        public bool Contains(Cube<T> other)
        {
            var tMax = Max;
            var oMax = other.Max;
            return Scalar.GreaterThanOrEqual(other.Origin.X, Origin.X) && Scalar.GreaterThanOrEqual
                (other.Origin.Y, Origin.Y) && Scalar.GreaterThanOrEqual
                (other.Origin.Z, Origin.Z) && Scalar.LessThanOrEqual(oMax.X, tMax.X) && Scalar.LessThanOrEqual
